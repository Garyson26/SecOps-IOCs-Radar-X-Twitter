import Image from 'next/image';
import React from 'react'
import { BiDotsVerticalRounded, BiRefresh, BiSolidPencil, BiZoomIn } from 'react-icons/bi';
import { useState } from 'react'
import { Switch } from '@headlessui/react'
import Link from 'next/link';
import { FiPlusCircle, FiSearch, FiTerminal } from 'react-icons/fi';
import { AiFillCaretDown, AiFillPlusCircle } from 'react-icons/ai';
import { FaPencilAlt, FaSearchPlus } from 'react-icons/fa';
import { RiDeleteBin6Fill } from 'react-icons/ri';
import { IoAddCircle, IoExtensionPuzzleSharp, IoReloadCircleSharp } from "react-icons/io5";
import { BsCheck, BsFillDatabaseFill, BsX } from 'react-icons/bs';
function classNames(...classes) {
  return classes.filter(Boolean).join(' ')
}

const orderdetail = [
  {
    id: '1',
    img: '/assets/catalog-1.webp',
    name: 'Hummingbird printed t-shirt',
    logo: '/assets/carrier_mini_2_1.jpg',
    ref: 'demo_1',
    Delay: 'Pick up in-store',
    status: true,
    freeShipping: true,
    category: 'Men',
    pricetex: '€27.30',
    pricewithouttext: '€27.30',
    value: '100'
  },

]
const Carriers = () => {
  const [enabled, setEnabled] = useState()
  const [opneModal, setOpenModal] = useState(false)
  return (
    <div>
      <div className='bg-white'>

        <div className=' flex items-center justify-between px-4 py-5 sm:px-6  lg:px-[15px] border-b '>
          <h1 className='text-[24px] font-semibold '>
            Carriers

          </h1>
          <div className='flex items-center gap-2'>
            <Link href="/shipping/carriers/add-carriers" className=' flex items-center border text-[14px] font-medium border-black text-black bg-white gap-1.5 px-3 py-1.5'>

              <AiFillPlusCircle className='text-[16px]' />  Add new Carriers
            </Link>

            <button className=' flex items-center border text-[14px] font-medium  text-black bg-white gap-1.5 px-3 py-1.5'>

              Help
            </button>
          </div>
        </div>

      </div>
      <div className='px-4 py-8 sm:px-6  lg:px-8'>

        <div className='border-t  bg-white border p-6 border-t-[#78c4d8]'>
          <div className='flex items-center justify-between'>
            <h2 className='text-[24px] text-[#363a41] font-semibold'>Carriers ({orderdetail.length})</h2>
            <div className='flex items-center gap-1'>
              <button>
                <IoAddCircle className='text-[18px] text-[#989ea7]' />
              </button>
              <button>
                <BiRefresh className='text-[20px] text-[#989ea7]' />
              </button>
              <button>

                <FiTerminal className='text-[18px] text-[#989ea7]' />
              </button>
              <button>
                <BsFillDatabaseFill className='text-[18px] text-[#989ea7]' />
              </button>

            </div>
          </div>

          <div className='mt-5 overflow-auto px-[4]'>
            <table className="min-w-full divide-y divide-gray-300">
              <thead>
                <tr>
                  <th scope="col" className="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-[#363a41] ">

                  </th>
                  <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-[#363a41]">
                    ID
                  </th>
                  <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-[#363a41]">
                    Name
                  </th>
                  <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-[#363a41]">
                    Logo
                  </th>
                  <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-[#363a41]">
                    Delay
                  </th>
                  <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-[#363a41]">
                    Status
                  </th>
                  <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-[#363a41]">
                    Free Shipping
                  </th>
                  <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-[#363a41]">
                    Position
                  </th>
                  <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-[#363a41]">

                  </th>

                </tr>
                <tr className='border-t border-[#78c4d8]'>
                  <td className='text-center bg-[#f7f7f7] p-2'>
                    --
                  </td>
                  <td className='p-2 max-w-[50px] bg-[#f7f7f7]' >
                    <input className='w-full' type='number' />
                  </td>
                  <td className='p-2 bg-[#f7f7f7]'>
                    <input type="text" className='w-full' />
                  </td>
                  <td className='text-center bg-[#f7f7f7] p-2'>
                    --
                  </td>
                  <td className='p-2 bg-[#f7f7f7]'>
                    <input type="text" className='w-full' />
                  </td>

                  <td className='text-center bg-[#f7f7f7] p-2'>
                    <select>
                      <option value={"0"}>
                        -
                      </option>
                      <option value={"Yes"}>
                        Yes
                      </option>
                      <option value={"No"}>
                        No
                      </option>
                    </select>
                  </td>
                  <td className='text-center bg-[#f7f7f7] p-2'>
                    <select>
                      <option value={"0"}>
                        -
                      </option>
                      <option value={"Yes"}>
                        Yes
                      </option>
                      <option value={"No"}>
                        No
                      </option>
                    </select>
                  </td>
                  <td className='p-2 bg-[#f7f7f7]'>
                    <input type='number' className='max-w-[100px]' />
                  </td>
                  <td className='text-end bg-[#f7f7f7] p-2'>
                    <button className='bg-black text-white ml-auto flex items-center px-4   p-2 gap-1'>
                      <FiSearch /> Search
                    </button>
                  </td>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                {orderdetail.map((item, index) => (
                  <tr key={index} className='even:bg-gray-50'>
                    <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 ">
                      <input type='checkbox' className='text-black ring-0 focus:ring-0 ' />
                    </td>
                    <td className="text-[#363a41] text-center p-2 text-[13px] ">{item.id}</td>

                    <td className="text-[#363a41] p-2 text-[13px]">{item.name}</td>
                    <td className="text-[#363a41] text-center p-2 text-[13px]">
                      <span className='block w-[34px] h-[34px] rounded border border-[#ccc] bg-white p-[4px]'>
                        <Image src={item.logo} width={32} height={32} className='max-w-full h-auto' />

                      </span>
                    </td>

                    <td className="text-[#363a41] p-2 text-[13px]">{item.Delay}</td>
                    <td className="text-[#363a41] text-center p-2 text-[13px]">
                      {item.status ?
                        <button className='text-center'>
                          <BsCheck className='text-green-600 text-[24px]' />


                        </button>
                        :
                        <button>
                          <BsX className='text-red-600 text-[24px] ' />
                        </button>
                      }</td>
                    <td className="text-[#363a41] p-2 text-center text-[13px]">  {item.freeShipping ?
                      <button className='text-center'>
                        <BsCheck className='text-green-600 text-[24px]' />


                      </button>
                      :
                      <button>
                        <BsX className='text-red-600 text-[24px] ' />
                      </button>
                    }</td>
                    <td className="text-[#363a41] p-2 text-[13px]">{index + 1}</td>

                    <td className='text-gray-500  p-2 ml-auto text-right'>
                      <div className="relative border whitespace-nowrap float-right   align-middle ">
                        <Link href="#" className="whitespace-nowrap pr-0 p-[8px] inline-flex items-center text-[14px]" title="View">
                          <FaPencilAlt className='mr-2' /> Edit
                        </Link>

                        <button onClick={() => { setEnabled(index + 1), setOpenModal(!opneModal) }} className="btn  p-[8px] btn-default relative float-none inline-flex items-center dropdown-toggle" data-toggle="dropdown">
                          <AiFillCaretDown />
                        </button>
                        <ul className={`${enabled === index + 1 && opneModal ? 'block' : 'hidden'} z-[2] absolute min-w-[160px] bg-white border p-[8px] right-0`}>


                          <li>
                            <Link href="#" title="Delete" className=" p-[8px] flex items-center justify-start gap-2" >
                              <RiDeleteBin6Fill /> Delete
                            </Link>
                          </li>
                        </ul>
                      </div>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </div>
      </div>
    </div>
  )
}

export default Carriers